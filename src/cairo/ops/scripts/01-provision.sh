...
# Usage:
#   source env.sh
#   bash src/cairo/ops/scripts/01-provision.sh
#
...
export_project_number() {
  ...
  if grep -q '^export PROJECT_NUMBER=' "${ENV_FILE}" 2>/dev/null; then
    sed -i.bak "s|^export PROJECT_NUMBER=.*|export PROJECT_NUMBER=\"${PROJECT_NUMBER}\"|" "${ENV_FILE}"
    rm -f "${ENV_FILE}.bak"
  else
    echo "" >> "${ENV_FILE}"
    echo "# Populated by src/cairo/ops/scripts/01-provision.sh" >> "${ENV_FILE}"
    echo "export PROJECT_NUMBER=\"${PROJECT_NUMBER}\"" >> "${ENV_FILE}"
  fi
