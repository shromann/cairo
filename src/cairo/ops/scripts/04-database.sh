...
# Usage:
#   source env.sh
#   bash src/cairo/ops/scripts/04-database.sh
#
...
export_connection_name() {
  ...
  if grep -q '^export SQL_CONNECTION=' "${ENV_FILE}" 2>/dev/null; then
    sed -i.bak "s|^export SQL_CONNECTION=.*|export SQL_CONNECTION=\"${SQL_CONNECTION}\"|" \
      "${ENV_FILE}"
    rm -f "${ENV_FILE}.bak"
  else
    echo "" >> "${ENV_FILE}"
    echo "# Populated by src/cairo/ops/scripts/04-database.sh" >> "${ENV_FILE}"
    echo "export SQL_CONNECTION=\"${SQL_CONNECTION}\"" >> "${ENV_FILE}"
  fi
...
enable_connection_pooling() {
  ...
  if gcloud sql instances patch "$SQL_INSTANCE" \
    ...
    warn "Workaround: cap --max-instances=10 on the web service in src/cairo/ops/scripts/08-smoke-test.sh."
...
main() {
  ...
  verify
  log ""
  log "Next step: run src/cairo/ops/scripts/05-secrets.sh to store DB_PASSWORD in Secret Manager."
