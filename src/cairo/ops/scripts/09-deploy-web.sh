...
#   source env.sh && src/cairo/ops/scripts/09-deploy-web.sh       # build + deploy
#   src/cairo/ops/scripts/09-deploy-web.sh --dry-run               # print commands only
# Requires steps 01-07 (APIs, IAM, bucket, Cloud SQL, secret, artifact registry).
# =============================================================================
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
: "${PROJECT_ID:?env.sh not sourced}"; : "${REGION:?}"; : "${REPO:?}"; : "${SQL_CONNECTION:?run src/cairo/ops/scripts/04-database.sh}"
: "${SQL_DB:=cairo}"; : "${SQL_USER:=cairo_app}"; : "${BUCKET_MEDIA:=${PROJECT_ID}-media}"
SERVICE="${SERVICE:-cairo-web}"
TAG="${TAG:-$(git -C "${REPO_ROOT}" rev-parse --short HEAD 2>/dev/null || date +%Y%m%d%H%M)}"
...
run gcloud builds submit "${REPO_ROOT}" --tag "$IMAGE" --timeout=1800 --machine-type=e2-highcpu-8
