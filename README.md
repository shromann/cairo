...
# Process a video
python -m cairo.backend.infer --study <study_accession_number>
```
...
│   ├── api.py                 # REST endpoints, health checks
│   ├── infer.py               # PanEcho inference + clip aggregation
│   ├── models.py              # SQLAlchemy ORM definitions
│   ├── repositories.py        # Database writes (idempotent)
│   ├── db.py                  # Connection pooling, session factory
│   └── config.py              # Environment config (GCP, paths, etc.)
...
# Edit env.sh with your GCP credentials, database URL, etc.
source env.sh
